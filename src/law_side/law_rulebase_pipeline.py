...
Pipeline stages:
0. Document ingest (two `.doc` files) -> `document_manifest.xlsx`
...
5. Rule seed construction -> `rulebase_seed.xlsx`
"""
...
class LawRulebasePipeline:
    """Orchestrate stages and export exactly 6 Excel files."""
    ...
    def from_yaml(cls, config_path: Path) -> "LawRulebasePipeline":
        ...
        if not doc_files:
            # Default two documents requested in the prompt.
    ...
    def run(self) -> dict[str, Path]:
        ...
        out_document_manifest = root / "data/raw/legal_corpus/manifest/document_manifest.xlsx"
        out_legal_units = root / "data/interim/law_parsing/legal_units_review.xlsx"
        out_candidate_sentences = root / "data/interim/law_parsing/candidate_normative_sentences.xlsx"
        out_legal_frames = root / "data/interim/law_parsing/legal_frames_review.xlsx"
        out_predicate_lexicon = root / "data/processed/ontology/predicate_lexicon.xlsx"
        out_rulebase_seed = root / "data/processed/rulebase/rulebase_seed.xlsx"
